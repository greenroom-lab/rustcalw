...
### 現状 (Phase 1 完了・Phase 2 完了・Phase 3 開始)
...
- ✅ Phase 2: 既存 gateway テスト 156/157 ファイル通過 (1550/1595 テスト、45 skipped)
- 🔧 Phase 3: 段階的 Rust 化 — E2E テスト疎通を維持しながらモジュール差し替え
...
Phase 3 進行中。gateway HTTP サーバースケルトン実装済み。

```bash
cd rust/
cargo build        # ビルド
cargo test         # ユニットテスト (335件 + live config 3件)
...
| gateway/        | gateway             | 型定義・スキーマ (25モジュール) + HTTP サーバースケルトン (health/ready エンドポイント) |
| cli/            | cli                 | config サブコマンド (check, path, deploy-check) + gateway サーバー起動 |
